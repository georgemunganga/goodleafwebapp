    ...
    if (USAGE_DEMO) {
      return {
        ...
        newDetails: {
          ...
          serviceCharge: 500,
          totalRepayment: 11050,
          repaymentMonths: request.newRepaymentMonths ?? 12,
          monthlyPayment: 11050 / (request.newRepaymentMonths ?? 12),
          status: 'active',
          approvalDate: new Date().toISOString(),
          firstPaymentDate: new Date().toISOString(),
          maturityDate: new Date(Date.now() + (request.extensionDays ?? 30) * 24 * 60 * 60 * 1000).toISOString(),
          nextPaymentDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
          amountPaid: 2750,
          amountRemaining: 8300,
          createdAt: new Date().toISOString(),
