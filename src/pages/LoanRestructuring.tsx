...
export default function LoanRestructuring() {
  ...
  const backTarget = activeLoan ? `/loans/${activeLoan.id}` : "/loans";

  const loanInfo = {
    loanId: activeLoan?.id || "GL-2025-001",
    currentTenure: activeLoan?.tenure || 12,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!reason || !explanation) {
      toast.error("Please provide a reason and explanation");
      return;
    }

    try {
      // Update the loan with restructuring request
      await updateLoan(loanInfo.loanId, {
        status: 'restructuring_requested',
        proposedTenure: parseInt(proposedTenure),
        restructuringReason: reason,
        restructuringExplanation: explanation
      });

      setIsSubmitted(true);
      toast.success("Loan restructuring request submitted successfully!");
    } catch (error) {
      console.error("Loan restructuring error:", error);
      toast.error("Failed to submit restructuring request. Please try again.");
    }
  };
        ...
        <main className="flex-1 px-4 py-6">
          <div className="bg-white rounded-2xl border border-slate-100 p-6 text-center">
            ...
            <p className="text-sm text-slate-600 mb-6">
              You need an active loan before you can request restructuring.
            </p>
